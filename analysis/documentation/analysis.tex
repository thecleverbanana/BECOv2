...
\subsection*{Zig-Zag Terrain}
\indent \indent Lets start with the Zig-Zag Terrain because it has a nice property that all end-point of legs should contact with the terrain.
Therefore, there would be only two particles \(A, B\) should be considered in this simlified case.  
The \textbf{DoF} of the system can be derived through the holonomic and non-holonomic constraint. 
\subsubsection*{Holonomic constraints:}
\[
\left\| \mathbf{r}_{B/O} - \mathbf{r}_{A/O} \right\| = L_\text{Spine} \cdot \sin(\arctan(\frac{L_\text{Spine}}{\abs{h2-h1}}))
\]

Let the inertial frame be defined as \( \mathcal{I} = \{ O; \mathbf{e}_x, \mathbf{e}_y \} \). The zig-zag terrain is defined by a periodic piecewise function \( y = h(x) \), with period \( T = 2r + d \) and height \( h = r \tan\theta \). For each point \( x \in \mathbb{R} \), define \( x_k = x \bmod T \). Then:
...
\end{cases}
\]
\indent The holonomic constraints for points A and B being on the terrain surface are given by:
\[
f_A(x_A, y_A) := y_A - h(x_A) = 0, \quad f_B(x_B, y_B) := y_B - h(x_B) = 0
\]

\subsubsection*{Non-Holonomic constraints:}
\indent \indent There are none.

\noindent Therefore, the total \textbf{DoF} is \(4-2 = 2\). And we can formulate 
