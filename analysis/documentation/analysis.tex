...
Therefore, we can express our 3 point explicitly as:

\[
\mathbf{p^\mathcal{I}}_A(q) =
\begin{bmatrix}
q \\
A \cdot \sin\left( \frac{2\pi}{T} q \right) + \frac{A}{2}
\end{bmatrix}
\]
...
\mathbf{f}_B(q) \\
h(\mathbf{f}_B(q))
\end{bmatrix}, \quad \text{by solving: } \\
(x_B-q)^2+(\mathbf{f}_B(x_B)-\mathbf{f}_B(q))^2 = L_\text{Arm}^2
\]

\[
\Rightarrow (x_B-q)^2+(A \cdot \sin\left( \frac{2\pi}{T} x_B \right)-
A \cdot \sin\left( \frac{2\pi}{T} q \right))^2 = L_\text{Arm}^2
\]
