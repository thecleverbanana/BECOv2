...
\(\mathbf{f}_{\beta}(\mathbf{p}_A^\mathcal{I},L_\text{Spine},L_{\text{Leg}1},L_{\text{Leg}2})\)
...
\[
\left( \mathbf{N}_A + \mathbf{N}_B 
+ \mathbf{f}_A + \mathbf{f}_B 
+ \mathbf{F}_g + \mathbf{F}_{\text{Spine}}
+ \mathbf{F}_{\text{Leg1}} + \mathbf{F}_{\text{Leg2}}  
- m\ddot{\mathbf{r}}_G \right) 
\cdot \delta \mathbf{r}_G = 0
\]

Now, lets firstly expand out the points. 
\[
\mathbf{p^\mathcal{I}}_A(q) =
\begin{bmatrix}
q \\
...
q \\
\tan\theta \cdot x_k
\end{bmatrix}, & x_k \in [0, r] \\[1em]
\begin{bmatrix}
q \\
\tan\theta \cdot (2r - x_k)
\end{bmatrix}, & x_k \in [r, 2r] \\[1em]
\begin{bmatrix}
q \\
0
\end{bmatrix}, & x_k \in [2r, 2r + d]
\end{cases}, \quad
x_k := q \bmod (2r + d)
\]
...
h(x_B(q))
\end{bmatrix}
\quad \text{where } x_B(q) \text{ satisfies:}
\]

\[
(x_B(q) - q)^2 + \left(h(x_B(q)) - h(q)\right)^2 = L_{\text{proj}}^2
\]
\[
h(x) = r \tan\theta \cdot \left(1 - 4 \left| \frac{x}{T} - \left\lfloor \frac{x}{T} + \frac{1}{2} \right\rfloor \right| \right)
\]
